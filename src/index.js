...
const resolveIdAsync = promisify(resolveId);

async function cachedReadFile (file, cb) {
	try {
		if (file in readFileCache === false) {
			readFileCache[file] = await readFileAsync(file);
		}
		cb(null, readFileCache[file]);
	} catch (e) {
...
async function cachedIsFile (file, cb) {
	if (file in isFileCache === false) {
		try {
			const stat = await statAsync(file);
			isFileCache[file] = stat.isFile();
		} catch (e) {
			if (e.code !== 'ENOENT') {
				delete isFileCache[file];
				cb(e);
			}
			isFileCache[file] = false;
		}
	}
	cb(null, isFileCache[file]);
}
	...
	return {
		...
		async resolveId (importee, importer) {
          ...
          browser[resolvedImportee + '.json']
			...
			const resolveOptions = {
				...
				packageFilter (pkg, pkgPath) {
					...
					if (useBrowserOverrides && typeof pkg['browser'] === 'object') {
						packageBrowserField = Object.keys(pkg['browser']).reduce(
							(browser, key) => {
			...
			try {
				let resolved = await resolveIdAsync(
					importee,
				...
				if (!disregardResult) {
					if (!preserveSymlinks && resolved) {
						const exists = await existsAsync(resolved);

						if (exists) {
							resolved = await realpathAsync(resolved);
						}
					}
                  ...
                  `behavior or 'preferBuiltins: true' to disable this warning`
						...
						jail &&
            resolved.indexOf(normalize(jail.trim(sep))) !== 0
					) {
						return null;
					}
				}
				...
				if (resolved && options.modulesOnly) {
					const code = await readFileAsync(resolved, 'utf-8');
