...
describe( 'rollup-plugin-node-resolve', function () {
	...
	it( 'disregards top-level syntax field', function () {
		return rollup.rollup({
	...
	it( 'allows use of the syntax.esmodules field', function () {
		return rollup.rollup({
	...
	it( 'allows use of object browser field, resolving nested directories', function () {
		return rollup.rollup({
			...
			assert.equal( module.exports.env, 'browser' );
			assert.equal( module.exports.dep, 'browser-dep' );
			assert.equal( module.exports.test, 41 );
		});
	});

	it( 'allows use of object syntax field, resolving nested directories', function () {
		return rollup.rollup({
			input: 'samples/syntax-object-nested/main.js',
			onwarn: expectNoWarnings,
			plugins: [
				nodeResolve({
					syntax: 'esmodules'
			...
			assert.equal( module.exports.dep, 'esmodules-dep' );
			assert.equal( module.exports.test, 42 );
		});
	});
